...
VOID Game_Paint(HWND hwnd)
{
    ...
    // 一个 for 循环 用七中不同的画笔绘制线条
    for (int i = 0; i <= 6; ++i)
    {
        y = (i + 1) * 70;

        SelectObject(g_hdc, g_hPen[i]); // 将对应的画笔选好
        MoveToEx(g_hdc, 30, y, NULL); // 将光标移动到(30, y)
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_GDIDEMOCORE));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_GDIDEMOCORE);
