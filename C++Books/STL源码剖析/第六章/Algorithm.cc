...
int main()
{
    ...
    // 面对有序区间(sorted range), 可以二分查找寻找某个元素.
    ...
    // 将 iv.begin() + 4 - iv.begin() 个元素排序, 放进 [iv.begin(), iv.begin() + 4] 内, 剩余元素不保证维持原相对次序.
    ...
    // 返回该子区间的头尾迭代器.
    // 如果没有这样的子区间, 返回的头尾迭代器均指向该特定元素可插入的地点.
    ...
    // 将小于 *(iv.begin() + 5)  (本例为 22) 的元素置于该元素之左.
    // 其余置于该元素之右, 不保证维持原来的相对位置.
    nth_element(iv.begin(), iv.begin() + 5, iv.end());
    copy(iv.begin(), iv.end(), ostream_iterator<int>(cout, " ")); // 20 17 12 17 22 22 23 30 30 40 33
    cout << endl;

    // 将大于 *(iv.begin() + 5) (本例为 22) 的元素置于该元素之左.
    nth_element(iv.begin(), iv.begin() + 5, iv.end(), greater<int>());
    copy(iv.begin(), iv.end(), ostream_iterator<int>(cout, " ")); // 30 33 40 30 23 22 20 22 17 12 17
    cout << endl;

    // 以 "是否符合 even() 条件" 为依据, 将符合者置于左段, 不符合置于右段.
    // 保证维持原来的相对位置, 如不需要, 则可改用 partition().
    stable_partition(iv.begin(), iv.end(), even());
    copy(iv.begin(), iv.end(), ostream_iterator<int>(cout, " ")); // 30 40 30 22 20 22 12 33 23 17 17
